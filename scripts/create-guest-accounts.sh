...
echo \$LDM_USERNAME | grep -E '^$basename[0-9]{1,3}\$' \\
    && ssh -S "\$LDM_SOCKET" "\$LDM_SERVER" 'sudo /usr/local/bin/reset-guest-account.sh'
EOF

# copy S00-guest-sessions to chroot as well
arch=$(uname -m)
if [[ ${arch:0:1} == 'i' ]]; then
    arch=i386
else
...
cp /usr/share/ldm/rc.d/S00-guest-sessions /opt/ltsp/$arch/usr/share/ldm/rc.d/
